...
module OCI
  ...
  class DatabaseManagement::Models::JobRun
    RUN_STATUS_ENUM = [
      RUN_STATUS_COMPLETED = 'COMPLETED'.freeze,
      RUN_STATUS_FAILED = 'FAILED'.freeze,
      RUN_STATUS_IN_PROGRESS = 'IN_PROGRESS'.freeze,
      RUN_STATUS_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    def run_status=(run_status)
      ...
      if run_status && !RUN_STATUS_ENUM.include?(run_status)
        OCI.logger.debug("Unknown value for 'run_status' [" + run_status + "]. Mapping to 'RUN_STATUS_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @run_status = RUN_STATUS_UNKNOWN_ENUM_VALUE
      else
