...
module OCI
  ...
  class Cims::Models::CreateResourceDetails
    ...
    # @option attributes [OCI::Cims::Models::CreateItemDetails] :item The value to assign to the {#item} property
    ...
    def region=(region)
      raise "Invalid value for 'region': this must be one of the values in REGION_ENUM." if region && !REGION_ENUM.include?(region)
    ...
    def availability_domain=(availability_domain)
      raise "Invalid value for 'availability_domain': this must be one of the values in AVAILABILITY_DOMAIN_ENUM." if availability_domain && !AVAILABILITY_DOMAIN_ENUM.include?(availability_domain)
