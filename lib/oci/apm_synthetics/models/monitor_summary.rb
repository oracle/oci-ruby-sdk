...
module OCI
  ...
  class ApmSynthetics::Models::MonitorSummary
    ...
    SCHEDULING_POLICY_ENUM = [
      ...
      SCHEDULING_POLICY_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    # **[Required]** Timeout in seconds. If isFailureRetried is true, then timeout cannot be more than 30% of repeatIntervalInSeconds time for monitors.
    ...
    def scheduling_policy=(scheduling_policy)
      # rubocop:disable Style/ConditionalAssignment
      if scheduling_policy && !SCHEDULING_POLICY_ENUM.include?(scheduling_policy)
        OCI.logger.debug("Unknown value for 'scheduling_policy' [" + scheduling_policy + "]. Mapping to 'SCHEDULING_POLICY_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @scheduling_policy = SCHEDULING_POLICY_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [id, display_name, monitor_type, vantage_points, vantage_point_count, script_id, script_name, status, repeat_interval_in_seconds, is_run_once, timeout_in_seconds, target, maintenance_window_schedule, time_created, time_updated, freeform_tags, defined_tags, is_run_now, scheduling_policy, batch_interval_in_seconds].hash
    end
