...
module OCI
  ...
  class CertificatesManagement::Models::UpdateCertificateAuthorityConfigDetails
    CONFIG_TYPE_ENUM = [
    ...
    def self.get_subtype(object_hash)
      ...
      return 'OCI::CertificatesManagement::Models::UpdateSubordinateCaIssuedByInternalCaConfigDetails' if type == 'SUBORDINATE_CA_ISSUED_BY_INTERNAL_CA'
      return 'OCI::CertificatesManagement::Models::UpdateRootCaByGeneratingInternallyConfigDetails' if type == 'ROOT_CA_GENERATED_INTERNALLY'
    ...
    # @option attributes [String] :stage The value to assign to the {#stage} property
    def initialize(attributes = {})
      ...
      self.stage = "PENDING" if stage.nil? && !attributes.key?(:'stage') # rubocop:disable Style/StringLiterals
    end
    ...
    def config_type=(config_type)
      raise "Invalid value for 'config_type': this must be one of the values in CONFIG_TYPE_ENUM." if config_type && !CONFIG_TYPE_ENUM.include?(config_type)
    ...
    def stage=(stage)
      raise "Invalid value for 'stage': this must be one of the values in STAGE_ENUM." if stage && !STAGE_ENUM.include?(stage)
