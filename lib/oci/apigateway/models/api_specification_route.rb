...
module OCI
  ...
  #
  class Apigateway::Models::ApiSpecificationRoute
    METHODS_ENUM = [
      METHODS_ANY = 'ANY'.freeze,
      METHODS_HEAD = 'HEAD'.freeze,
      METHODS_GET = 'GET'.freeze,
      METHODS_POST = 'POST'.freeze,
      METHODS_PUT = 'PUT'.freeze,
      METHODS_PATCH = 'PATCH'.freeze,
      METHODS_DELETE = 'DELETE'.freeze,
      METHODS_OPTIONS = 'OPTIONS'.freeze,
      METHODS_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    # **[Required]** A URL path pattern that must be matched on this route. The path pattern may contain a subset of RFC 6570 identifiers
    ...
    # @return [OCI::Apigateway::Models::ApiSpecificationRouteBackend]
    ...
    def initialize(attributes = {})
      ...
      self.methods = attributes[:'methods'] if attributes[:'methods']
      ...
      self.backend = attributes[:'backend'] if attributes[:'backend']
    end
    # rubocop:enable Metrics/CyclomaticComplexity, Metrics/AbcSize, Metrics/PerceivedComplexity
    # rubocop:enable Metrics/MethodLength, Layout/EmptyLines, Style/SymbolLiteral

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] methods Object to be assigned
    def methods=(methods)
      ...
      if methods.nil?
        ...
        @methods =
          methods.collect do |item|
            if METHODS_ENUM.include?(item)
              item
            else
              OCI.logger.debug("Unknown value for 'methods' [#{item}]. Mapping to 'METHODS_UNKNOWN_ENUM_VALUE'") if OCI.logger
              METHODS_UNKNOWN_ENUM_VALUE
            end
          end
      end
