...
module OCI
  ...
  class IdentityDomains::Models::AccountRecoverySetting
    ...
    # @option attributes [Array<String>] :factors The value to assign to the {#factors} property
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] factors Object to be assigned
    def factors=(factors)
      # rubocop:disable Style/ConditionalAssignment
      if factors.nil?
        @factors = nil
        ...
        @factors =
          factors.collect do |item|
            if FACTORS_ENUM.include?(item)
              item
            else
              OCI.logger.debug("Unknown value for 'factors' [#{item}]. Mapping to 'FACTORS_UNKNOWN_ENUM_VALUE'") if OCI.logger
              FACTORS_UNKNOWN_ENUM_VALUE
            end
          end
      end
      # rubocop:enable Style/ConditionalAssignment
