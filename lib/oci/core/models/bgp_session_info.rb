...
module OCI
  ...
  class Core::Models::BgpSessionInfo
    BGP_STATE_ENUM = [
    ...
    #
    # @return [String]
    attr_accessor :oracle_interface_ip
    ...
    # @param [Object] bgp_state Object to be assigned
    def bgp_state=(bgp_state)
      # rubocop:disable Style/ConditionalAssignment
      if bgp_state && !BGP_STATE_ENUM.include?(bgp_state)
        OCI.logger.debug("Unknown value for 'bgp_state' [" + bgp_state + "]. Mapping to 'BGP_STATE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @bgp_state = BGP_STATE_UNKNOWN_ENUM_VALUE
      else
        @bgp_state = bgp_state
      end
