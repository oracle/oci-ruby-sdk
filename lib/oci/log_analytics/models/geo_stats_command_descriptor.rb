...
module OCI
  ...
  #
  class LogAnalytics::Models::GeoStatsCommandDescriptor < LogAnalytics::Models::AbstractCommandDescriptor
    INCLUDE_ENUM = [
      INCLUDE_CLIENT = 'CLIENT'.freeze,
      INCLUDE_SERVER = 'SERVER'.freeze,
      INCLUDE_CLIENT_AND_SERVER = 'CLIENT_AND_SERVER'.freeze,
      INCLUDE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    #
    # @return [Array<OCI::LogAnalytics::Models::FunctionField>]
    ...
    # @option attributes [String] :include The value to assign to the {#include} property
    # @option attributes [Array<OCI::LogAnalytics::Models::AbstractField>] :group_by_fields The value to assign to the {#group_by_fields} property
    # @option attributes [Array<OCI::LogAnalytics::Models::FunctionField>] :functions The value to assign to the {#functions} property
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      attributes['name'] = 'GEO_STATS'
      ...
      self.group_by_fields = attributes[:'groupByFields'] if attributes[:'groupByFields']

      raise 'You cannot provide both :groupByFields and :group_by_fields' if attributes.key?(:'groupByFields') && attributes.key?(:'group_by_fields')

      self.group_by_fields = attributes[:'group_by_fields'] if attributes[:'group_by_fields']
    ...
    def include=(include)
      ...
      if include && !INCLUDE_ENUM.include?(include)
        OCI.logger.debug("Unknown value for 'include' [" + include + "]. Mapping to 'INCLUDE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @include = INCLUDE_UNKNOWN_ENUM_VALUE
      else
        @include = include
      end
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        group_by_fields == other.group_by_fields &&
    ...
    def hash
      [name, display_query_string, internal_query_string, category, referenced_fields, declared_fields, include, group_by_fields, functions].hash
    end
