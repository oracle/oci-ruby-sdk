...
module OCI
  ...
  #
  class DataSafe::Models::SqlFirewallConfig
    STATUS_ENUM = [
    ...
    # **[Required]** Specifies whether Data Safe should automatically purge the violation logs
    ...
    def violation_log_auto_purge=(violation_log_auto_purge)
      # rubocop:disable Style/ConditionalAssignment
      if violation_log_auto_purge && !VIOLATION_LOG_AUTO_PURGE_ENUM.include?(violation_log_auto_purge)
        OCI.logger.debug("Unknown value for 'violation_log_auto_purge' [" + violation_log_auto_purge + "]. Mapping to 'VIOLATION_LOG_AUTO_PURGE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @violation_log_auto_purge = VIOLATION_LOG_AUTO_PURGE_UNKNOWN_ENUM_VALUE
      else
    ...
    def exclude_job=(exclude_job)
      # rubocop:disable Style/ConditionalAssignment
      if exclude_job && !EXCLUDE_JOB_ENUM.include?(exclude_job)
        OCI.logger.debug("Unknown value for 'exclude_job' [" + exclude_job + "]. Mapping to 'EXCLUDE_JOB_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @exclude_job = EXCLUDE_JOB_UNKNOWN_ENUM_VALUE
      else
