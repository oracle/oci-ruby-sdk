...
module OCI
  ...
  class Devops::Models::Trigger
    ...
    def trigger_source=(trigger_source)
      ...
      if trigger_source && !TRIGGER_SOURCE_ENUM.include?(trigger_source)
        OCI.logger.debug("Unknown value for 'trigger_source' [" + trigger_source + "]. Mapping to 'TRIGGER_SOURCE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @trigger_source = TRIGGER_SOURCE_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [id, display_name, description, project_id, compartment_id, trigger_source, time_created, time_updated, lifecycle_state, lifecycle_details, actions, freeform_tags, defined_tags, system_tags].hash
    end
