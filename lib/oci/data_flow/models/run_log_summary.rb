...
module OCI
  ...
  #
  class DataFlow::Models::RunLogSummary
    SOURCE_ENUM = [
      SOURCE_APPLICATION = 'APPLICATION'.freeze,
      SOURCE_DRIVER = 'DRIVER'.freeze,
      SOURCE_EXECUTOR = 'EXECUTOR'.freeze,
      SOURCE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    TYPE_ENUM = [
      TYPE_STDERR = 'STDERR'.freeze,
      TYPE_STDOUT = 'STDOUT'.freeze,
    ...
    # @option attributes [String] :run_id The value to assign to the {#run_id} property
    ...
    def initialize(attributes = {})
      ...
      self.run_id = attributes[:'runId'] if attributes[:'runId']

      raise 'You cannot provide both :runId and :run_id' if attributes.key?(:'runId') && attributes.key?(:'run_id')

      self.run_id = attributes[:'run_id'] if attributes[:'run_id']
    ...
    def hash
      [name, run_id, size_in_bytes, source, time_created, type].hash
    end
