...
module OCI
  ...
  class Database::Models::ConvertToPdbTargetBase
    TARGET_ENUM = [
    ...
    def self.get_subtype(object_hash)
      type = object_hash[:'target'] # rubocop:disable Style/SymbolLiteral

      return 'OCI::Database::Models::PdbConversionToNewDatabaseDetails' if type == 'NEW_DATABASE'
    ...
    def target=(target)
      raise "Invalid value for 'target': this must be one of the values in TARGET_ENUM." if target && !TARGET_ENUM.include?(target)
