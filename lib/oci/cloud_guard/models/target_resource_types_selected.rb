...
require 'date'
require 'logger'
require_relative 'target_selected'

# rubocop:disable Lint/UnneededCopDisableDirective, Metrics/LineLength
module OCI
  # Target selection on basis of TargetResourceTypes.
  class CloudGuard::Models::TargetResourceTypesSelected < CloudGuard::Models::TargetSelected
    VALUES_ENUM = [
      ...
      if values.nil?
        ...
        @values =
          values.collect do |item|
            if VALUES_ENUM.include?(item)
              item
            else
              OCI.logger.debug("Unknown value for 'values' [#{item}]. Mapping to 'VALUES_UNKNOWN_ENUM_VALUE'") if OCI.logger
              VALUES_UNKNOWN_ENUM_VALUE
            end
          end
      end
