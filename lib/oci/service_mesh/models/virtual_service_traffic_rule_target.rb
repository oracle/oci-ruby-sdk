...
require 'date'
require 'logger'
require_relative 'traffic_rule_target'

# rubocop:disable Lint/UnneededCopDisableDirective, Metrics/LineLength
module OCI
  # Traffic router target for an ingress gateway.
  class ServiceMesh::Models::VirtualServiceTrafficRuleTarget < ServiceMesh::Models::TrafficRuleTarget
    ...
    def initialize(attributes = {})
      ...
      attributes['type'] = 'VIRTUAL_SERVICE'
      ...
      self.port = 9090 if port.nil? && !attributes.key?(:'port') # rubocop:disable Style/StringLiterals

      self.weight = attributes[:'weight'] if attributes[:'weight']
      self.weight = 1 if weight.nil? && !attributes.key?(:'weight') # rubocop:disable Style/StringLiterals
    end
    ...
    def hash
      [type, virtual_service_id, port, weight].hash
    end
