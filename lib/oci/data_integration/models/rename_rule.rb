...
module OCI
  ...
  class DataIntegration::Models::RenameRule < DataIntegration::Models::ProjectionRule
    ...
    # @option attributes [String] :from_name The value to assign to the {#from_name} property
    # @option attributes [String] :to_name The value to assign to the {#to_name} property
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      attributes['modelType'] = 'RENAME_RULE'
      ...
      self.from_name = attributes[:'fromName'] if attributes[:'fromName']

      raise 'You cannot provide both :fromName and :from_name' if attributes.key?(:'fromName') && attributes.key?(:'from_name')

      self.from_name = attributes[:'from_name'] if attributes[:'from_name']

      self.to_name = attributes[:'toName'] if attributes[:'toName']

      raise 'You cannot provide both :toName and :to_name' if attributes.key?(:'toName') && attributes.key?(:'to_name')

      self.to_name = attributes[:'to_name'] if attributes[:'to_name']
    end
    ...
    def hash
      [model_type, key, model_version, parent_ref, is_java_regex_syntax, config_values, object_status, description, is_skip_remaining_rules_on_match, from_name, to_name].hash
    end
