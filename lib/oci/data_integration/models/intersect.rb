...
module OCI
  ...
  class DataIntegration::Models::Intersect < DataIntegration::Models::Operator
    INTERSECT_TYPE_ENUM = [
      INTERSECT_TYPE_NAME = 'NAME'.freeze,
      INTERSECT_TYPE_POSITION = 'POSITION'.freeze,
      INTERSECT_TYPE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    # intersectType
    # @return [String]
    attr_reader :intersect_type
    ...
    # @option attributes [String] :intersect_type The value to assign to the {#intersect_type} property
    # @option attributes [BOOLEAN] :is_all The value to assign to the {#is_all} property
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      attributes['modelType'] = 'INTERSECT_OPERATOR'
      ...
      self.intersect_type = attributes[:'intersectType'] if attributes[:'intersectType']

      raise 'You cannot provide both :intersectType and :intersect_type' if attributes.key?(:'intersectType') && attributes.key?(:'intersect_type')

      self.intersect_type = attributes[:'intersect_type'] if attributes[:'intersect_type']
    ...
    # @param [Object] intersect_type Object to be assigned
    def intersect_type=(intersect_type)
      # rubocop:disable Style/ConditionalAssignment
      if intersect_type && !INTERSECT_TYPE_ENUM.include?(intersect_type)
        OCI.logger.debug("Unknown value for 'intersect_type' [" + intersect_type + "]. Mapping to 'INTERSECT_TYPE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @intersect_type = INTERSECT_TYPE_UNKNOWN_ENUM_VALUE
      else
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        intersect_type == other.intersect_type &&
        is_all == other.is_all
    end
    ...
    def hash
      [model_type, key, model_version, parent_ref, name, description, object_version, input_ports, output_ports, object_status, identifier, parameters, op_config_values, intersect_type, is_all].hash
    end
