...
module OCI
  ...
  class DataIntegration::Models::Minus < DataIntegration::Models::Operator
    MINUS_TYPE_ENUM = [
      MINUS_TYPE_NAME = 'NAME'.freeze,
      MINUS_TYPE_POSITION = 'POSITION'.freeze,
      MINUS_TYPE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    # minusType
    # @return [String]
    attr_reader :minus_type
    ...
    # @option attributes [String] :minus_type The value to assign to the {#minus_type} property
    # @option attributes [BOOLEAN] :is_all The value to assign to the {#is_all} property
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      attributes['modelType'] = 'MINUS_OPERATOR'
      ...
      self.minus_type = attributes[:'minusType'] if attributes[:'minusType']

      raise 'You cannot provide both :minusType and :minus_type' if attributes.key?(:'minusType') && attributes.key?(:'minus_type')

      self.minus_type = attributes[:'minus_type'] if attributes[:'minus_type']
    ...
    # @param [Object] minus_type Object to be assigned
    def minus_type=(minus_type)
      # rubocop:disable Style/ConditionalAssignment
      if minus_type && !MINUS_TYPE_ENUM.include?(minus_type)
        OCI.logger.debug("Unknown value for 'minus_type' [" + minus_type + "]. Mapping to 'MINUS_TYPE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @minus_type = MINUS_TYPE_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [model_type, key, model_version, parent_ref, name, description, object_version, input_ports, output_ports, object_status, identifier, parameters, op_config_values, minus_type, is_all].hash
    end
