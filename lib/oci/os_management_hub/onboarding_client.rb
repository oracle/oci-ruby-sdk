...
module OCI
  ...
  class OsManagementHub::OnboardingClient
    ...
    # @option opts [Array<String>] :profile_type A filter to return registration profiles that match the given profileType.
    #   Allowed values are: SOFTWARESOURCE, GROUP, LIFECYCLE, STATION
    # @option opts [String] :profile_id The OCID of the registration profile.
    ...
    # @option opts [String] :vendor_name A filter to return only profiles that match the given vendorName.
    ...
    #   Example: `3`
    #
    # @option opts [String] :lifecycle_state A filter to return only registration profile whose lifecycleState matches the given lifecycleState.
    ...
    def list_profiles(opts = {})
      ...
      profile_type_allowable_values = %w[SOFTWARESOURCE GROUP LIFECYCLE STATION]
      if opts[:profile_type] && !opts[:profile_type].empty?
        opts[:profile_type].each do |val_to_check|
          unless profile_type_allowable_values.include?(val_to_check)
            raise 'Invalid value for "profile_type", must be one of SOFTWARESOURCE, GROUP, LIFECYCLE, STATION.'
          end
        end
      end
