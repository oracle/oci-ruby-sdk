...
module OCI
  ...
  class Oda::Models::UpdateOsvcChannelDetails < Oda::Models::UpdateChannelDetails
    ...
    def channel_service=(channel_service)
      raise "Invalid value for 'channel_service': this must be one of the values in CHANNEL_SERVICE_ENUM." if channel_service && !CHANNEL_SERVICE_ENUM.include?(channel_service)
    ...
    def hash
      [description, type, session_expiry_duration_in_milliseconds, freeform_tags, defined_tags, host, port, user_name, password, total_session_count, channel_service, authentication_provider_name, bot_id].hash
    end
