...
module OCI
  ...
  class OperatorAccessControl::AccessRequestsClient
    ...
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    # @option opts [Integer] :enable_process_tree To enable process tree computation in audit report (default to 0)
    # @option opts [String] :opc_request_id The client request ID for tracing.
    # @return [Response] A Response object with data of type {OCI::OperatorAccessControl::Models::AuditLogReport AuditLogReport}
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/operatoraccesscontrol/get_audit_log_report.rb.html) to see an example of how to use get_audit_log_report API.
    def get_audit_log_report(access_request_id, opts = {})
      logger.debug 'Calling operation AccessRequestsClient#get_audit_log_report.' if logger

      raise "Missing the required parameter 'access_request_id' when calling get_audit_log_report." if access_request_id.nil?
      raise "Parameter value for 'access_request_id' must not be blank" if OCI::Internal::Util.blank_string?(access_request_id)

      path = '/accessRequests/{accessRequestId}/auditLogReport'.sub('{accessRequestId}', access_request_id.to_s)
      operation_signing_strategy = :standard

      # rubocop:disable Style/NegatedIf
      # Query Params
      query_params = {}
      query_params[:enableProcessTree] = opts[:enable_process_tree] if opts[:enable_process_tree]
      ...
      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'AccessRequestsClient#get_audit_log_report') do
        @api_client.call_api(
          :GET,
          path,
          endpoint,
          ...
          operation_signing_strategy: operation_signing_strategy,
          body: post_body,
          return_type: 'OCI::OperatorAccessControl::Models::AuditLogReport'
        )
      end
      # rubocop:enable Metrics/BlockLength
    end
