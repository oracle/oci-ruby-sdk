...
module OCI
  ...
  class Loggingsearch::LogSearchClient
    ...
    def initialize(config: nil, region: nil, endpoint: nil, signer: nil, proxy_settings: nil, retry_config: nil)
      ...
      if endpoint
        @endpoint = endpoint + '/20190909'
      else
        region ||= config.region
        region ||= signer.region if signer.respond_to?(:region)
        self.region = region
      end
      logger.info "LogSearchClient endpoint set to '#{@endpoint}'." if logger
    end
    ...
    def region=(new_region)
      ...
      @endpoint = OCI::Regions.get_service_endpoint_for_template(@region, 'https://logging.{region}.oci.{secondLevelDomain}') + '/20190909'
      logger.info "LogSearchClient endpoint set to '#{@endpoint} from region #{@region}'." if logger
    end
    ...
    def search_logs(search_logs_details, opts = {})
      ...
      raise "Missing the required parameter 'search_logs_details' when calling search_logs." if search_logs_details.nil?
      ...
      post_body = @api_client.object_to_http_body(search_logs_details)

      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'LogSearchClient#search_logs') do
        @api_client.call_api(
          :POST,
          path,
          endpoint,
          ...
          body: post_body,
          return_type: 'OCI::Loggingsearch::Models::SearchResponse'
        )
      end
