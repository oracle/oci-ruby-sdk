...
module OCI
  ...
  class Logging::Models::UnifiedJSONParser < Logging::Models::UnifiedAgentParser
    TIME_TYPE_ENUM = [
      ...
      TIME_TYPE_UNIXTIME = 'UNIXTIME'.freeze,
      TIME_TYPE_STRING = 'STRING'.freeze,
    ...
    # @option attributes [String] :time_type The value to assign to the {#time_type} property
    # @option attributes [String] :time_format The value to assign to the {#time_format} property
    def initialize(attributes = {})
      return unless attributes.is_a?(Hash)

      attributes['parserType'] = 'JSON'
      ...
      self.time_type = attributes[:'timeType'] if attributes[:'timeType']

      raise 'You cannot provide both :timeType and :time_type' if attributes.key?(:'timeType') && attributes.key?(:'time_type')

      self.time_type = attributes[:'time_type'] if attributes[:'time_type']
    ...
    def time_type=(time_type)
      ...
      if time_type && !TIME_TYPE_ENUM.include?(time_type)
        OCI.logger.debug("Unknown value for 'time_type' [" + time_type + "]. Mapping to 'TIME_TYPE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @time_type = TIME_TYPE_UNKNOWN_ENUM_VALUE
      else
        @time_type = time_type
      end
    ...
    def hash
      [parser_type, field_time_key, types, null_value_pattern, is_null_empty_string, is_estimate_current_event, is_keep_time_key, timeout_in_milliseconds, time_type, time_format].hash
    end
